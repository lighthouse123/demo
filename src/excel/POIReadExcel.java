...
public class POIReadExcel {
    ...
    public static void main(String[] args) {
        String string = readExcelToHtml("C:\\Users\\admin\\Desktop\\平台车.xlsx",true);
...
//		}
        ...
        // 这个map数组 里面有四个map
        map = getRowSpanColSpanMap(sheet);   ////保存合并单元格的对应起始和截止单元格 第一个
...
//				}
                ...
                if (stringValue == null || "".equals(stringValue.trim())) {
                    sb.append("   ");
                    if( colNum == lastColNum-2) {
                        sb.append("<input type='text' name='check"+(rowNum - 2)+"'>");
                    }else if(colNum == lastColNum-1) {
                        sb.append("<textarea rows='3' cols='20' style='resize: none;' name='remark"+(rowNum - 2)+"'></textarea>");
                    }
                } else {
                    // 将ascii码为160的空格转换为html下的空格（ ）  只是替换
...
//				}
                ...
                if(map[0].containsKey(topRow + "," + topColumn)){//该单元格为合并单元格，宽度需要获取所有单元格宽度后合并
                    ...
                    if(topColumn != bottomColumn){//合并列，需要计算相应宽度
                        ...
                        sb.append("width:" + 2*columnWidth / 256 * hf.getFontHeight() / 20 + "pt;");
                    }else{
                        int columnWidth = sheet.getColumnWidth(cell.getColumnIndex()) ;
                        sb.append("width:" + 2*columnWidth / 256 * hf.getFontHeight() / 20 + "pt;");
                    }
    ...
    private static int[] getColsOfTable(Sheet sheet) {
        ...
        int[] data = {0, 0};
        // 获取工作表上的第一行  返回 工作表上第一个逻辑行数(基于0)
        for(int i = sheet.getFirstRowNum(); i < sheet.getLastRowNum(); i ++) {
            if(null != sheet.getRow(i)) {
                //                        表示行的最后一个逻辑单元格加1，如果该行不包含任何单元格，则为-1。
